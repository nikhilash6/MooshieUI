  ...
  {#if notifications.panelOpen}
    ...
    >
      <div class="flex items-center justify-between px-4 py-3 border-b border-neutral-800">
        <h3 class="text-sm font-semibold text-neutral-100">Notifications</h3>
        {#if notifications.hasUnread}
          <button
      ...
      <div class="divide-y divide-neutral-800">
        ...
        {:else}
          {#each notifications.notifications as notif (notif.id)}
            <button
              class="w-full text-left px-4 py-3 hover:bg-neutral-800/50 transition-colors {notif.read ? 'opacity-60' : ''}"
